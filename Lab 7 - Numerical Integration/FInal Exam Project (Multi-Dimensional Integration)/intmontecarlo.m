function [fmed, fdev, N] = intmontecarlo(f, chi, boxlb, boxub, modo, stopcrit, rngconfig)
% Integração de Monte Carlo da função f na região R
%
...
while loopcond(N, fdev/abs(fmed))
    ...
    acum_f = acum_f + f(x)*min(chi(x));
    acum_fsqr = acum_fsqr + (f(x)*min(chi(x))).^2;
    ...
    fmed = (vol)*acum_f/N;
    fdev = (vol)*sqrt( (acum_fsqr/N - (acum_f/N).^2) / N);
end
